...
const cloudflareAccessHeaders = { "cf-access-authenticated-user-email": "admin@techmaxxed.com" };
const cloudflareAccessPage = await module.default.fetch(new Request("https://admin.test/admin/testing-functions/", { headers: cloudflareAccessHeaders }), env);
assert.equal(cloudflareAccessPage.status, 200);
assert.match(await cloudflareAccessPage.text(), /admin@techmaxxed\.com/);
