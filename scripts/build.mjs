...
import { checkoutConfig, checkoutOfferForProduct, checkoutOffers, checkoutPathForProduct } from "../content/checkout-data.mjs";
...
function toolPage(product, familyLabel) {
  ...
  const offer = checkoutOfferForProduct(product.slug);
  const checkoutButton = offer ? `<a class="button" href="../../${checkoutPathForProduct(offer.id)}">Buy ${escapeHtml(product.name)}</a>` : `<a class="button" href="../../support/?app=${encodeURIComponent(product.name)}&issue=${encodeURIComponent("Feature request")}">Request buying info</a>`;
  ...
  return `<header class="site-header">
    <div class="nav-shell">
      ...
      <nav class="nav-links" id="primary-nav" data-nav-links data-open="false" aria-label="Primary navigation">
        ...
        <a class="nav-button" href="${makeLink(depth, "checkout/")}">Checkout</a>
      </nav>
    </div>
  </header>`;
}
...
function footer(depth, makeLink = link) {
  return `<footer class="site-footer">
    <div class="shell footer-grid">
      ...
      <div><h2>Products</h2><ul><li><a href="${makeLink(depth, "apps/")}">All products</a></li><li><a href="${makeLink(depth, "pricing/")}">Pricing</a></li><li><a href="${makeLink(depth, "checkout/")}">Checkout</a></li><li><a href="${makeLink(depth, "plugins/")}">WordPress tools</a></li><li><a href="${makeLink(depth, "apps/maxxed-remote/")}">Maxxed Remote</a></li><li><a href="${makeLink(depth, "apps/maxxed-compass/")}">Maxxed Compass</a></li><li><a href="${makeLink(depth, "apps/rival-rush/")}">Rival Rush</a></li></ul></div>
...
function offerCard(offer, depth = 0) {
  return `<article class="price-card" data-checkout-card data-category="${escapeHtml(offer.family.toLowerCase().replaceAll(" ", "-"))}">
    <div><span class="status">${escapeHtml(offer.label)}</span><h3>${escapeHtml(offer.name)}</h3><p>${escapeHtml(offer.summary)}</p></div>
    <div><strong class="price">${money(offer.price)}</strong><span class="fine-print">${escapeHtml(offer.billing)}</span></div>
    <div class="fact-row"><span>${escapeHtml(offer.family)}</span><span>${escapeHtml(offer.status)}</span></div>
    <div class="hero-actions"><a class="button" href="${link(depth, checkoutPathForProduct(offer.id))}">Start checkout</a><a class="button secondary" href="${link(depth, offer.productPath)}">Product details</a></div>
  </article>`;
}

function pricingPage() {
  const androidOffers = checkoutOffers.filter((offer) => offer.family === "Android app");
  const wordpressOffers = checkoutOffers.filter((offer) => offer.family !== "Android app");
  const faqs = [
    ["Do you collect card numbers on TechMaxxed.com?", "No. Checkout buttons route to hosted payment pages or a manual invoice request. Card details are handled by the payment processor."],
    ["What happens after purchase?", "Fulfillment depends on the product. Android products may use Google Play or tester access, while WordPress tools receive download and setup instructions."],
    ["Can I buy before a public launch?", "Only when the product page and checkout flow clearly state the current access level. Development-stage access may be fulfilled as early access or manual setup."],
    ["Can I request a refund?", "Email support with the product, purchase email, order reference, and issue. Refund handling depends on the store or payment processor used."],
  ];
  const body = `<section class="band"><div class="shell section compact"><p class="eyebrow">Pricing</p><h1>Buy Maxxed apps and tools</h1><p class="lede">Start with launch app pricing, early-access Android products, and WordPress tools. Payments use hosted checkout links or manual invoicing, so TechMaxxed.com never asks for card details.</p><div class="proof-row"><span>${checkoutOffers.length} checkout-ready offers</span><span>Hosted payment links</span><span>Manual invoice fallback</span><span>Support: ${site.email}</span></div><div class="hero-actions"><a class="button" href="../checkout/">Open checkout</a><a class="button secondary" href="../apps/">Browse products</a></div></div></section>
  <section class="shell section"><div class="section-head"><div><p class="eyebrow">Android</p><h2>Launch app pricing</h2></div><p>Android purchases are tied to release state, Google Play access, device compatibility, and fulfillment instructions.</p></div><div class="price-grid">${androidOffers.map((offer) => offerCard(offer, 1)).join("")}</div></section>
  <section class="band"><div class="shell section"><div class="section-head"><div><p class="eyebrow">WordPress</p><h2>Tools and bundles</h2></div><p>WordPress products are review-first workflow tools. Fulfillment can include download links, setup notes, and support routing after payment confirmation.</p></div><div class="price-grid">${wordpressOffers.map((offer) => offerCard(offer, 1)).join("")}</div></section>
  <section class="shell section compact"><div class="faq-list">${faqs.map(([question, answer]) => `<details><summary>${escapeHtml(question)}</summary><p>${escapeHtml(answer)}</p></details>`).join("")}</div></section>`;
  const schema = [
    { "@context": "https://schema.org", "@type": "CollectionPage", name: "Maxxed product pricing", url: canonical("pricing/"), mainEntity: { "@type": "ItemList", numberOfItems: checkoutOffers.length, itemListElement: checkoutOffers.map((offer, index) => ({ "@type": "ListItem", position: index + 1, name: offer.name, url: canonical(`checkout/?product=${offer.id}`) })) } },
    faqSchema(faqs),
  ];
  return layout({ title: "Pricing", description: "View pricing for Maxxed Android apps, WordPress tools, bundles, early access, hosted checkout, and purchase support.", path: "pricing/", depth: 1, current: "pricing", body, schema });
}

function checkoutPage() {
  const offersJson = JSON.stringify(checkoutOffers).replaceAll("<", "\\u003c");
  const options = checkoutOffers.map((offer) => `<option value="${escapeHtml(offer.id)}">${escapeHtml(offer.name)} - ${money(offer.price)}</option>`).join("");
  const body = `<section class="band"><div class="shell section compact"><p class="eyebrow">Secure purchase routing</p><h1>Checkout</h1><p class="lede">Choose a product, review fulfillment notes, then continue to hosted checkout. If a payment link is not live yet, the same flow prepares a purchase request for manual invoicing.</p><div class="proof-row"><span>${checkoutConfig.processorLabel}</span><span>No card fields on this site</span><span>Manual invoice fallback</span><span>${site.email}</span></div></div></section>
  <section class="shell section"><div class="checkout-shell"><form class="checkout-panel" data-checkout-form data-email="${site.email}"><div class="field"><label for="checkout-product">Product</label><select id="checkout-product" name="product" data-checkout-select>${options}</select></div><div class="checkout-summary" data-checkout-summary aria-live="polite"></div><div class="consent-list"><label><input type="checkbox" name="purchaseConsent" required><span>I understand availability, device compatibility, fulfillment method, and refund handling can vary by product and payment processor.</span></label></div><div class="form-actions"><a class="button" data-checkout-action href="mailto:${site.email}">Continue to hosted checkout</a><a class="button secondary" data-checkout-support href="mailto:${site.email}">Ask before buying</a><p class="form-status" data-checkout-status>${escapeHtml(checkoutConfig.processorNote)}</p></div><noscript><p>Email ${site.email} with the product name to request a checkout link or invoice.</p></noscript></form><aside class="checkout-panel secondary-panel"><p class="eyebrow">How fulfillment works</p><h2>Payment first, access second</h2><p>After payment confirmation, fulfillment is handled according to the product: Google Play or tester access for Android apps, download and setup notes for WordPress tools, or manual confirmation for bundles.</p><p class="fine-print">${escapeHtml(checkoutConfig.fallbackNote)}</p></aside></div></section>
  <script type="application/json" id="checkout-offers">${offersJson}</script>`;
  const schema = [{ "@context": "https://schema.org", "@type": "CheckoutPage", name: "Maxxed checkout", url: canonical("checkout/"), description: "Hosted checkout routing for Maxxed Technical Systems products." }];
  return layout({ title: "Checkout", description: "Choose a Maxxed product and continue to hosted checkout or request manual invoicing without entering card details on TechMaxxed.com.", path: "checkout/", depth: 1, current: "pricing", body, schema });
}
...
function adminCatalogRecords() {
  const appRecords = apps.map((app) => ({
    id: app.slug,
    name: app.name,
    route: `https://techmaxxed.com/apps/${app.slug}/`,
...
function productPage(app) {
  const offer = checkoutOfferForProduct(app.slug);
  const checkoutButton = offer ? `<a class="button" href="../../${checkoutPathForProduct(app.slug)}">Buy ${escapeHtml(app.name)}</a>` : `<a class="button" href="../../pricing/">View pricing</a>`;
...
function pluginPage(plugin) {
  ...
  const offer = checkoutOfferForProduct(plugin.slug);
  const checkoutButton = offer ? `<a class="button" href="../../${checkoutPathForProduct(plugin.slug)}">Buy ${escapeHtml(plugin.name)}</a>` : `<a class="button" href="../../pricing/">View pricing</a>`;
...
async function adminTestingFunctionsHtml() {
  ...
  return (await response.text())
    ...
    .replaceAll('href="/admin/builds/"', 'href="/products/"')
    .replaceAll('href="/admin/catalog/"', 'href="/products/"')
    .replaceAll('href="/admin/security/"', 'href="/"')
    .replaceAll('href="/admin/settings/"', 'href="/"')
    ...
    .replace(
      ...
      "<main><p class=\"eyebrow\">QA operations</p><div class=\"empty\" style=\"margin-bottom:16px\"><strong>Admin control center</strong><br>Static export loaded from /public_html/admin. The full UI is present; APK uploads, runner jobs, artifacts, and evidence require the Worker admin backend. <a href=\"/testing-functions/\">Testing Functions</a></div>",
    );
}
...
await writePage("pricing/index.html", pricingPage());
await writePage("checkout/index.html", checkoutPage());
