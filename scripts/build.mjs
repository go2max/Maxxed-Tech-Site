...
import { toolProducts } from "../content/tool-products.mjs";
...
const allProducts = [...apps, ...wordpressPlugins, ...toolProducts, ...repoProducts, ...powerhouseProducts];
...
function staticProductCard(item, depth = 0, type = "Repo-backed product") {
  const body = `<div class="app-card-top"><span class="app-icon" aria-hidden="true">${escapeHtml(item.icon)}</span><span class="status">${escapeHtml(item.status)}</span></div><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml(item.summary)}</p><div class="fact-row">${(item.facts || []).map((fact) => `<span>${escapeHtml(fact)}</span>`).join("")}</div><span class="app-meta">${escapeHtml(item.ctaLabel || type)}${item.route ? " -&gt;" : ""}</span>`;
  if (item.route) return `<a class="app-card" data-app-card data-category="${cardCategory(item, "tool utility")}" style="--accent:${escapeHtml(item.accent)}" href="${link(depth, item.route)}">${body}</a>`;
  return `<article class="app-card" data-app-card data-category="${cardCategory(item, "repo utility")}" style="--accent:${escapeHtml(item.accent)}">${body}</article>`;
}

function header(depth, current, makeLink = link) {
  const nav = [["apps", "Apps", "apps/"], ["plugins", "Plugins", "plugins/"], ["beta", "Beta", "beta/"], ["roadmap", "Roadmap", "roadmap/"], ["support", "Help", "support/"]];
...
function footer(depth, makeLink = link) {
  return `<footer class="site-footer"><p>${escapeHtml(site.name)}. Practical software for Android, WordPress, browser tools, and repo-backed products.</p><nav aria-label="Footer"><a href="${makeLink(depth, "apps/")}">Apps</a><a href="${makeLink(depth, "plugins/")}">Plugins</a><a href="${makeLink(depth, "privacy/")}">Privacy</a><a href="${makeLink(depth, "terms/")}">Terms</a><a href="${makeLink(depth, "accessibility/")}">Accessibility</a></nav></footer>`;
}
...
function catalogFilters() {
  const filters = [["all", "All"], ["utility", "Utilities"], ["tool", "Tools"], ["outdoors", "Outdoors"], ["games", "Games"], ["wordpress", "WordPress"], ["repo", "Standalone repos"], ["powerhouse", "Powerhouse repos"], ["business", "Business"], ["civic", "Civic"], ["content", "Content"], ["finance", "Finance"]];
...
function homePage() {
  ...
  const toolHighlights = toolProducts.slice(0, 3);
  const repoHighlights = [...repoProducts.slice(0, 3), ...powerhouseProducts.slice(0, 3)];
  const body = `<section class="hero"><p class="eyebrow">Android apps + WordPress plugins + browser tools + repo products</p><h1>Maxxed Technical Systems</h1><p>${escapeHtml(site.description)}</p><div class="hero-actions"><a class="button" href="apps/">Explore all products</a><a class="button secondary" href="plugins/">WordPress plugins</a></div><div class="proof-row"><span>${allProducts.length} public products</span><span>${apps.length} Android apps</span><span>${wordpressPlugins.length} WordPress plugins</span><span>${toolProducts.length} browser tools</span><span>${repoProducts.length + powerhouseProducts.length} repo products</span></div></section><section class="shell section"><div class="section-head"><div><p class="eyebrow">Android apps</p><h2>Built around real tasks</h2></div><p>Focused app products stay prominent while the full repo-backed and browser-tool catalog remains searchable.</p></div><div class="app-grid">${apps.map((app, index) => appCard(app, 0, index < 2)).join("")}</div></section><section class="shell section"><div class="section-head"><div><p class="eyebrow">Browser tools</p><h2>First-class web utilities</h2></div><p>Small, useful tools are listed directly in the catalog with live routes and support paths.</p></div><div class="app-grid">${toolHighlights.map((tool) => staticProductCard(tool, 0, "Browser tool")).join("")}</div></section><section class="band" data-plugin-summary><div class="shell section"><div class="section-head"><div><p class="eyebrow">WordPress plugin lab</p><h2>${wordpressPlugins.length} plugins prepared for testing</h2></div><p>Every checked WordPress plugin package is listed separately for lab review.</p></div><div class="app-grid">${pluginHighlights.map((plugin) => staticProductCard(plugin, 0, "WordPress plugin package")).join("")}</div><div class="hero-actions"><a class="button secondary" href="plugins/">View all ${wordpressPlugins.length} WordPress plugins</a></div></div></section><section class="shell section"><div class="section-head"><div><p class="eyebrow">Repo-backed product library</p><h2>${repoProducts.length + powerhouseProducts.length} additional repo products</h2></div><p>Standalone repos and materialized powerhouse repos are included without flooding the homepage.</p></div><div class="app-grid">${repoHighlights.map((product) => staticProductCard(product, 0)).join("")}</div><div class="hero-actions"><a class="button secondary" href="apps/">Browse all ${allProducts.length} products</a></div></section>${contactBand(0)}`;
...
function appsPage() {
  const cards = `${apps.map((app) => appCard(app, 1)).join("")}${toolProducts.map((tool) => staticProductCard(tool, 1, "Browser tool")).join("")}${wordpressPlugins.map((plugin) => staticProductCard(plugin, 1, "WordPress plugin package")).join("")}${repoProducts.map((product) => staticProductCard(product, 1)).join("")}${powerhouseProducts.map((product) => staticProductCard(product, 1)).join("")}`;
  const body = `<section class="band"><div class="shell section compact"><p class="eyebrow">Product directory</p><h1>Android apps, browser tools, WordPress plugins, and software</h1><p class="lede">Browse ${allProducts.length} public products, including ${apps.length} Android apps, ${toolProducts.length} browser tools, ${wordpressPlugins.length} WordPress plugins, ${repoProducts.length} standalone repo products, and ${powerhouseProducts.length} powerhouse repo products.</p></div></section><section class="shell section">${catalogFilters()}<div class="app-grid" data-catalog>${cards}</div><p class="empty-state" data-empty-state hidden>No products match that search. Try a product name or broader category.</p></section>${contactBand(1)}`;
  return layout({ title: "Apps", description: "Browse Maxxed Technical Systems Android apps, browser tools, WordPress plugins, standalone repo products, and powerhouse repo products in one organized catalog.", path: "apps/", depth: 1, current: "apps", body });
}

function pluginsPage() {
  const body = `<section class="band"><div class="shell section compact"><p class="eyebrow">WordPress plugin lab</p><h1>WordPress plugins</h1><p class="lede">Browse every checked Maxxed WordPress plugin package prepared for lab testing and organized plugin review.</p><div class="proof-row"><span>${wordpressPlugins.length} plugin packages</span><span>Plugin-owned admin workflows</span><span>Installed artifact ready</span><span>Public catalog cards</span></div></div></section><section class="shell section"><div class="catalog-tools"><label class="search-box"><span class="skip-link">Search plugins</span><input type="search" data-app-search placeholder="Search plugins by name or capability" autocomplete="off"></label><div class="filters" role="group" aria-label="Filter plugins"><button class="filter" data-filter="all" aria-pressed="true">All</button><button class="filter" data-filter="wordpress" aria-pressed="false">WordPress</button></div></div><p class="fine-print" data-result-count aria-live="polite"></p><div class="app-grid" data-catalog>${wordpressPlugins.map((plugin) => staticProductCard(plugin, 1, "WordPress plugin package")).join("")}</div><p class="empty-state" data-empty-state hidden>No plugins match that search.</p></section>${contactBand(1, "Need help with a WordPress plugin?")}`;
...
function toolPage(tool) {
  const body = `<section class="band product-hero"><div class="shell product-hero-grid"><div><div class="product-kicker" style="--accent:${escapeHtml(tool.accent)}"><span class="app-icon" aria-hidden="true">${escapeHtml(tool.icon)}</span><span class="status">${escapeHtml(tool.status)}</span></div><p class="eyebrow">${escapeHtml(tool.category)} tool</p><h1>${escapeHtml(tool.name)}</h1><p class="lede">${escapeHtml(tool.description)}</p><div class="fact-row">${tool.facts.map((fact) => `<span>${escapeHtml(fact)}</span>`).join("")}</div><div class="hero-actions"><a class="button" href="${link(2, tool.appRoute)}">${escapeHtml(tool.ctaLabel || "Open tool")}</a><a class="button secondary" href="${link(2, "support/")}">Get support</a></div></div><div class="product-visual" style="--accent:${escapeHtml(tool.accent)}"><span class="app-icon" aria-hidden="true">${escapeHtml(tool.icon)}</span><strong>${escapeHtml(tool.tagline)}</strong></div></div></section><section class="shell section"><div class="section-head"><div><p class="eyebrow">Capabilities</p><h2>What ${escapeHtml(tool.short)} does</h2></div><p>${escapeHtml(tool.summary)}</p></div><div class="feature-list" style="--accent:${escapeHtml(tool.accent)}">${tool.features.map(([title, text], index) => `<article class="feature-item"><span class="feature-number">0${index + 1}</span><h3>${escapeHtml(title)}</h3><p>${escapeHtml(text)}</p></article>`).join("")}</div></section><section class="shell section"><div class="truth-grid" style="--accent:${escapeHtml(tool.accent)}"><article class="truth-item"><h3>Availability</h3><p>${escapeHtml(tool.availability)}</p></article><article class="truth-item"><h3>Privacy</h3><p>${escapeHtml(tool.privacy)}</p></article><article class="truth-item"><h3>Important limitation</h3><p>${escapeHtml(tool.limitation)}</p></article></div></section>${contactBand(2, `Need help with ${tool.name}?`)}`;
  return layout({ title: tool.name, description: tool.summary, path: tool.route, depth: 2, current: "apps", body, schema: [{ "@context": "https://schema.org", "@type": "SoftwareApplication", name: tool.name, applicationCategory: "BusinessApplication", operatingSystem: "Web browser", description: tool.summary }] });
}
...
function adminPage() {
  return layout({ title: "Admin Routing", description: "Private routing hub for Maxxed Technical Systems product catalog review, plugin package checks, beta, support, roadmap, and policy travel.", path: "admin/", depth: 1, current: "", noIndex: true, body: `<section class="band"><div class="shell section compact"><p class="eyebrow">Private operations</p><h1>Maxxed internal routing</h1><p class="lede">A direct control hub for product catalog review, plugin package checks, beta flow, support, and release planning.</p></div></section><section class="shell section"><div class="app-grid"><a class="app-card" href="plugins/"><h2>Plugin packages</h2><p>Review WordPress plugin package readiness without customer-site settings routes.</p></a><a class="app-card" href="../apps/"><h2>Public catalog</h2><p>Open all ${allProducts.length} listed products.</p></a><a class="app-card" href="../plugins/"><h2>Public plugins</h2><p>Open all ${wordpressPlugins.length} plugin cards.</p></a></div></section>` });
}

function adminPluginsPage() {
  return layout({ title: "WordPress Plugin Review", description: "Private review view of checked WordPress plugin packages without customer-site profile or settings page claims.", path: "admin/plugins/", depth: 2, current: "", noIndex: true, body: `<section class="band"><div class="shell section compact"><p class="eyebrow">Plugin packages</p><h1>WordPress plugin package review</h1><p class="lede">Every local WordPress plugin repo is tracked for package readiness. Customer-facing configuration belongs inside each plugin's own WordPress admin workflow.</p></div></section><section class="shell section"><div class="app-grid">${wordpressPlugins.map((plugin) => staticProductCard(plugin, 2, "Plugin package review")).join("")}</div></section>` });
}

function roadmapPage() {
  return simplePage("Roadmap", "roadmap/", "roadmap", "Follow the Tech Maxxed release queue for active Android apps, WordPress plugins, browser tools, and repo-backed product catalog work.", `<section class="band"><div class="shell section compact"><p class="eyebrow">Roadmap</p><h1>Product release queue</h1><p class="lede">Statuses describe current work, not availability promises.</p></div></section><section class="shell section"><div class="road-list">${roadmap.map((item, index) => `<div class="road-item"><span class="number">${String(index + 1).padStart(2, "0")}</span><div><strong>${escapeHtml(item[0])}</strong><p>${escapeHtml(item[2])}</p></div><span class="kind">${escapeHtml(item[1])}</span></div>`).join("")}</div></section>`);
}
...
for (const tool of toolProducts) {
  await writeHtml(tool.route.replace(/\/$/, ""), toolPage(tool));
}
...
await writeHtml("about", simplePage("About", "about/", "about", "Learn how Maxxed Technical Systems builds practical Android apps, WordPress plugins, browser tools, and repo-backed software products.", `<section class="band"><div class="shell section compact"><p class="eyebrow">About</p><h1>About Maxxed Technical Systems</h1><p class="lede">A practical product studio building focused Android apps, WordPress tools, browser utilities, and repo-backed software products.</p></div></section>${contactBand(1)}`));
await writeHtml("support", simplePage("Help & Support", "support/", "support", "Get help and support for Maxxed Technical Systems apps, plugins, browser tools, beta access, and product questions.", `<section class="band"><div class="shell section compact"><p class="eyebrow">Support</p><h1>Help &amp; Support</h1><p class="lede">Email <a href="mailto:${site.email}">${site.email}</a> for product support, plugin lab questions, tool support, and beta access.</p></div></section>`));
...
const indexedPaths = ["", "apps/", "plugins/", ...apps.flatMap((app) => [`apps/${app.slug}/`, `apps/${app.slug}/privacy/`]), ...toolProducts.map((tool) => tool.route), "roadmap/", "about/", "support/", "privacy/", "accessibility/", "beta/", "beta-credits/", "terms/", "tools/post-purge-pro/"];
...
/admin/*
  ...
  Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'
`);

await mkdir(resolve(root, "admin"), { recursive: true });
await writeFile(resolve(root, "admin/_headers"), `/*
  X-Robots-Tag: noindex, nofollow, noarchive
  Cache-Control: no-store
  X-Content-Type-Options: nosniff
  Referrer-Policy: no-referrer
  Permissions-Policy: accelerometer=(), camera=(), geolocation=(), gyroscope=(), magnetometer=(), microphone=(), payment=(), usb=()
  Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://api-admin.techmaxxed.com; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'
`);
